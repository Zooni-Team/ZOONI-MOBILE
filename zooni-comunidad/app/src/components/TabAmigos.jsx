    ...
    <FlatList
      ...
      renderItem={({ item }) => (
        <View style={styles.item}>
          <View style={[styles.avatar, { borderColor: item.online ? '#2DBD72' : '#AAAAAA' }]}>
            <Text style={{ fontSize: 20 }}>👤</Text>
          </View>
          <View style={styles.info}>
            <Text style={styles.nombre}>{item.nombre}</Text>
            <Text style={styles.sub}>
              {item.mascota_nombre || 'Sin mascota'}
              {item.distancia_km != null ? `  ·  ${item.distancia_km} km` : ''}
            </Text>
          </View>
          <TouchableOpacity
            style={styles.btnVer}
            onPress={() => onVerEnMapa(item)}
            accessibilityLabel={`Ver ${item.nombre} en el mapa`}
          >
